/* Beau's Game Inventory — Smart Scan */
(function(){'use strict';
...
function normalise(data,barcode){
 ...
 const valid=rawSources.filter(x=>!String(x.source||x.store||x.name||'').toLowerCase().includes('cash converter')&&!String(x.source||x.store||x.name||'').toLowerCase().includes('cashconverters')&&Number(x.price||x.sale_price)>0);
 const sources=valid.map(x=>({source:x.source||x.store||x.name||'Source',price:Number(x.price||x.sale_price)}));
 const r=retail||Math.max(0,...sources.map(x=>x.price).filter(Number.isFinite));
 const m=market||0;
