...
(function(){
  ...
  const siteOf=s=>{const x=sourceName(s);if(/cash\s*converters|cashies|cashies/.test(x))return 'Cash Converters';if(/e\s*-?bay/.test(x))return 'eBay';if(/super\s*retro/.test(x))return 'Super Retro';return ''};
  ...
  const averages=(raw,n)=>{
    ...
    const addFrom=(obj)=>{const site=siteOf(obj);if(site)buckets[site].push(...pricesFor(obj));};
    addFrom(raw?.product||{});addFrom(raw?.pricing||{});
    const out={};for(const [site,vals] of Object.entries(buckets)){const u=vals.filter(v=>v>0);out[site]=u.length?{average:u.reduce((a,b)=>a+b,0)/u.length,count:u.length,prices:u}:null}
    const present=Object.values(out).filter(Boolean).map(v=>v.average);out.overall=present.length?present.reduce((a,b)=>a+b,0)/present.length:0;
  ...
  async function renderDecision(raw,barcode){
    ...
    panel.innerHTML='<h3>♻️ Second-Hand Pricing</h3>'+
      ...
      '<h4>Site Averages</h4>'+row('Cash Converters',sites['Cash Converters'])+row('eBay',sites.eBay)+row('Super Retro',sites['Super Retro'])+
      ...
      '<div class="muted">Each site's available price samples are averaged first. The overall market figure is the mean of the available site averages, so one site with many listings cannot dominate the result. Super Retro prices are pre-owned unless a product is marked otherwise.</div>'+ 
