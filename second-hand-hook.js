...
(function(){
  ...
  const cleanPlatform=p=>{const s=String(p||'').trim(); if(!s)return ''; return s.replace(/^Nintendo\s+/i,'Nintendo ')};
  ...
  const urls=query=>({
    ...
    ebay:'https://www.ebay.com.au/sch/i.html?_nkw='+encodeURIComponent(query)
  });
  let lastBarcode='';
  async function renderDecision(raw,barcode){
    ...
    const title=String(n.title||'').trim(), platform=cleanPlatform(n.platform||'');
    const query=buildQuery(title,platform), link=urls(query);
    const resale=Number(n.second)||Number(n.resale)||0;
    const guide=(n.buyGuide&&typeof n.buyGuide==='object')?n.buyGuide:(eng&&typeof eng.buyGuide==='function'?eng.buyGuide(resale,'Used - Good'):{});
    ...
    panel.innerHTML='<h3>♻️ Second-Hand Pricing</h3>'+
      ...
      '<div class="list-item"><span>Used-market estimate</span><b>'+money(resale)+'</b></div>'+
      ...
      '<div class="toolbar"><a class="btn primary" href="'+link.cash+'" target="_blank" rel="noopener">🔎 Cash Converters: '+esc(query||'Search game')+'</a><a class="btn" href="'+link.ebay+'" target="_blank" rel="noopener">🔎 eBay AU: '+esc(query||'Search game')+'</a></div>'+
    ...
    if(btn&&asking&&decision)btn.onclick=()=>{const a=Number(asking.value)||0;if(!a){decision.textContent='Enter a seller price to get BUY / MAYBE / PASS.';return}const target=twentyFive||Number(guide.recommended)||0,max=forty||Number(guide.maximum)||0;let label='❌ PASS';if(target>0&&a<=target)label='✅ BUY';else if(max>0&&a<=max)label='🟡 MAYBE';decision.innerHTML='<b>'+label+'</b> · Asking '+money(a)+' · Recommended '+money(target)+' · Maximum '+money(max)};
  }
  const install=()=>{
    ...
    const guardedSecondHand=async barcode=>{
      const b=norm(barcode); if(!b)return;
      if(lastBarcode===b&&document.getElementById('secondHandPricingPanel'))return;
      lastBarcode=b;
      let raw=null;
      try{raw=await window.BeauPricingEngine.lookup(b);await renderDecision(raw.data||raw,b)}catch(_){}
      try{return await originalSecondHand(b)}catch(_){}
    };
    window.searchSecondHandPricing=guardedSecondHand;
    const wrapped=async function(code){
      const b=norm(code); const result=await originalLookup.apply(this,arguments);
      if(b)try{await guardedSecondHand(b)}catch(_){}
      return result;
    };
