/* Beau's Game Inventory — robust web OTA updater */
(function(){
  ...
  function reloadLatest(manifestUrl){const hash=rememberScreen();try{localStorage.setItem('beauLastScreenHash',hash);}catch(e){}const base=manifestUrl||location.href.split('#')[0];const u=new URL(base,location.href);u.searchParams.set('otaVersion',Date.now().toString());u.hash=hash;window.location.href=u.toString();}
