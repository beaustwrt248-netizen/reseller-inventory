...
(function(){
  ...
  function compare(a,b){
    const aa=String(a).replace(/^v/,'').split('.').map(Number),bb=String(b).replace(/^v/,'').split('.').map(Number);
  ...
  async function check(){
    ...
    return {current:CURRENT,latest:String(d.version||CURRENT),available:compare(d.version||CURRENT,CURRENT)>0,notes:d.message||'',url:d.url||OTA_URL};
  }
  ...
  async function checkAndUpdate(options={}){
    try{
      ...
      if(result.available){
        const ok=options.silent?true:confirm('A new version of Beau\'s Game Inventory is available.\n\nVersion '+result.latest+'\n'+(result.notes||'')+'\n\nUpdate now?');
        if(ok)reloadLatest();
  ...
  window.addEventListener('load',()=>{setTimeout(()=>checkAndUpdate({silent:true}),1500)});
})();
