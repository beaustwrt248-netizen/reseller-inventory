...
(function(){
  ...
  function reloadLatest(targetUrl){
    const currentHash=location.hash||'';
    const base=targetUrl||location.href;
    const u=new URL(base,location.href);
    u.searchParams.set('ota',Date.now());
    if(currentHash) u.hash=currentHash;
    sessionStorage.setItem('beauReturnHash',currentHash);
    sessionStorage.setItem('beauOtaReload','1');
  ...
  try{
    ...
    if(previous&&previous!==CURRENT){
      ...
      n.innerHTML='<strong>✨ Beau\'s Game Inventory updated</strong><br><span style="opacity:.85">The latest app version is now loaded.</span> <button id="updateDismiss" style="float:right;border:0;border-radius:8px;padding:7px 10px;font-weight:bold">OK</button>';
      document.body.appendChild(n);document.getElementById('updateDismiss').onclick=()=>n.remove();
    }
