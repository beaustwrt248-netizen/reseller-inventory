...
(function(){
  ...
  function compare(a,b){
    ...
    for(let i=0;i<Math.max(aa.length,bb.length);i++){const x=aa[i]||0,y=bb[i]||0;if(x!==y)return x-y} return 0;
  }
  ...
  window.BeauUpdate={version:CURRENT,check,reloadLatest,checkAndUpdate}; try{localStorage.setItem(KEY,CURRENT)}catch(e){} window.addEventListener('load',()=>{setTimeout(()=>checkAndUpdate({silent:true}),1500)});
})();