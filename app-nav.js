/* Beau's Game Inventory v1.9.1 — navigation-safe sectioned app */
(function(){
'use strict';
...
function pageForPanel(panel){
 ...
 if(t.includes('quick sale')||t.includes('sales history')||t.includes('record sale')||t==='sales')return 'sales';
...
function buildPages(){
 ...
 if(!c)return;
 const existing=c.querySelectorAll(':scope > .app-page');
...
function watchDynamicPanels(){
 ...
 setTimeout(()=>{moveNewPanels();},250);
 setTimeout(()=>{moveNewPanels();},1000);
 setTimeout(()=>{moveNewPanels();},2500);
}
function ensureSettings(){
 ...
 if(!document.getElementById('appSettingsPanel')){
  ...
  s.innerHTML='<h2>⚙️ Settings</h2><p>Manage updates, appearance, backup and administration.</p><div class="price-box"><h3>🔄 App Updates</h3><p id="settingsVersion">Current version: checking…</p><p id="settingsUpdateStatus">Check for the latest web app version.</p><div class="actions"><button class="primary" id="settingsCheckUpdates">🔎 Check for Updates</button><button class="primary" id="settingsLoadUpdate" style="display:none">⬇️ Load Latest Version</button></div></div><div class="price-box"><h3>🎨 Appearance</h3><button class="secondary" id="settingsThemeToggle">🌙 Toggle Dark Mode</button></div><div class="price-box"><h3>📱 App Information</h3><div class="price-row"><span>App</span><strong>Beau\'s Game Inventory</strong></div><div class="price-row"><span>Version</span><strong>v1.9.1</strong></div></div>';
  p.appendChild(s);
 }
 wireSettings();
 load('beauAdminPanel','./admin-panel.js?v=1.7.1');
 if(!document.getElementById('beauAdminCss')){const l=document.createElement('link');l.id='beauAdminCss';l.rel='stylesheet';l.href='./admin-panel.css?v=1.7.1';document.head.appendChild(l)}
}
function wireSettings(){
 ...
 v.textContent='Current version: v'+(window.BeauUpdate?.version||'1.9.1');
 ...
 th.onclick=()=>{document.body.classList.toggle('dark-mode');localStorage.setItem('beauDarkMode',document.body.classList.contains('dark-mode')?'1':'0')};
...
function activate(k,transition){
 ...
 document.querySelectorAll('.mobile-nav button').forEach(b=>b.classList.toggle('active',b.dataset.screen===k));
...
function mount(){
 ...
 n.innerHTML=items.map(x=>`<button type="button" data-screen="${x[2]}"><span class="icon">${x[0]}</span><span class="label">${x[1]}</span></button>`).join('');
 ...
 load('beauSmartToolsScript','./smart-reseller.js?v=1.9.1');
 load('beauSmartScanScript','./smart-scan.js?v=1.9.1');
 load('beauSmartScanCss','./smart-scan.css?v=1.9.1');
 load('beauPriceComparisonCleanup','./price-comparison-cleanup.js?v=1.9.1');
 load('beauProToolsScript','./pro-tools.js?v=1.9.1');
 load('beauWelcomeCss','./welcome-tutorial.css?v=1.9.1');
 load('beauWelcomeTutorial','./welcome-tutorial.js?v=1.9.1');
 load('beauNavLayoutFix','./navigation-layout-fix.css?v=1.9.1');
}
